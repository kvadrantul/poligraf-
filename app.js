...
async function generateImage(prompt, referenceImage) {
    ...
    if (referenceImage) {
        // Если есть референс: используем image-to-image с КОРОТКИМ промптом (CLIP лимит 77 токенов!)
        // ВАЖНО: промпт должен быть коротким, чтобы не обрезаться CLIP токенизатором
        // Референс - это главное, промпт только уточняет детали
        // Сокращаем промпт до минимума, чтобы уложиться в 77 токенов
        imagePrompt = `extract graphics from reference, ${englishPrompt}, white background, high quality`;
        ...
        console.log('  - Instruction: Short prompt to fit CLIP 77 token limit, reference image is primary');
    } else {
    ...
    try {
        ...
        if (lastHTML && lastHTML.length > 100) {
            ...
            if (isValidHTML) {
                ...
                // Формируем промпт: "возьми за основу вот этот HTML и используй изображение как паттерн на весь фон"
...
ВАЖНО: Используй изображение которое я прикрепил как основной паттерн/узор на весь фон. Максимально используй это изображение - сделай его доминирующим элементом дизайна. Используй именно это изображение, а не похожие цвета или элементы. Расположи изображение так, чтобы оно покрывало весь фон и было хорошо видно. И сделай ${prompt}`;
                } else {
                    userPrompt += `возьми за основу вот этот HTML:
...
и сделай ${prompt}`;
                ...
                if (generatedImage || uploadedImageBase64) {
                    userPrompt += `ВАЖНО: Используй изображение которое я прикрепил как основной паттерн/узор на весь фон. Максимально используй это изображение - сделай его доминирующим элементом дизайна. Используй именно это изображение, а не похожие цвета или элементы. Расположи изображение так, чтобы оно покрывало весь фон и было хорошо видно. И сделай ${prompt}`;
                } else {
            ...
            if (generatedImage || uploadedImageBase64) {
                // ВАЖНО: v0.dev получает изображение через image_url в messages
                // Нужно явно указать использовать именно это изображение, а не генерировать placeholder
                // v0.dev может использовать переменную или прямое обращение к прикрепленному изображению
                userPrompt += `КРИТИЧЕСКИ ВАЖНО: В этом сообщении прикреплено изображение. Ты ДОЛЖЕН использовать ИМЕННО ЭТО прикрепленное изображение как фон. НЕ создавай placeholder изображения, НЕ используй data:image/png;base64,iVBORw0KGgo... или другие placeholder. Используй прикрепленное изображение напрямую в коде. Сделай это изображение доминирующим элементом дизайна, покрывающим весь фон через backgroundImage или <img>. И сделай ${prompt}`;
            } else {
                userPrompt += `сделай ${prompt}`;
            }
        }
