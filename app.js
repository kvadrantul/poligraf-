...
async function generateImage(prompt, referenceImage) {
    ...
    if (referenceImage) {
        // Если есть референс: используем image-to-image с промптом, который явно указывает на сохранение визуальных элементов
        // ВАЖНО: промпт должен явно указывать на сохранение и извлечение конкретных графических элементов из референса
        // Референс - это главное, промпт должен подчеркивать необходимость сохранения визуальных элементов
        ...
        console.log('  - Instruction: Explicit prompt to preserve and extract exact graphics from reference');
    } else {
...
и сделай ${prompt}`;
            ...
            if (generatedImage || uploadedImageBase64) {
                userPrompt += `ВАЖНО: Используй изображение которое я прикрепил как основной паттерн/узор на весь фон. Максимально используй это изображение - сделай его доминирующим элементом дизайна. Используй именно это изображение, а не похожие цвета или элементы. Расположи изображение так, чтобы оно покрывало весь фон и было хорошо видно. ВАЖНО: Используй именно те графические элементы, которые видны на изображении (например, если там розы - используй розы, если там узоры - используй эти узоры). И сделай ${prompt}`;
            } else {
                userPrompt += `сделай ${prompt}`;
            }
        }
