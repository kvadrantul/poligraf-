...
и сделай ${prompt}`;
            ...
            if (generatedImage || uploadedImageBase64) {
                userPrompt += `КРИТИЧЕСКИ ВАЖНО: Я прикрепил изображение. Ты ДОЛЖЕН использовать ИМЕННО ЭТО изображение как фон/паттерн. НЕ создавай новый SVG паттерн, НЕ генерируй похожие элементы - используй ТОЧНО ТО изображение, которое я прикрепил. Используй его через CSS background-image с data URL или через <img> тег. Сделай это изображение доминирующим элементом дизайна, покрывающим весь фон. И сделай ${prompt}`;
            } else {
                userPrompt += `сделай ${prompt}`;
            }
        }
