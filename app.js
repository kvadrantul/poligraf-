...
и сделай ${prompt}`;
            ...
            if (generatedImage || uploadedImageBase64) {
                ...
                userPrompt += `КРИТИЧЕСКИ ВАЖНО: Я прикрепил изображение в этом сообщении. Ты ДОЛЖЕН использовать ИМЕННО ЭТО изображение (которое прикреплено) как фон. НЕ создавай placeholder, НЕ используй data:image/png;base64,iVBORw0KGgo... - используй ТОЧНО ТО изображение, которое я прикрепил. Используй его через style={{backgroundImage: 'url(ПРИКРЕПЛЕННОЕ_ИЗОБРАЖЕНИЕ)'}} или через <img src={ПРИКРЕПЛЕННОЕ_ИЗОБРАЖЕНИЕ} />. Сделай это изображение доминирующим элементом, покрывающим весь фон. И сделай ${prompt}`;
            } else {
                userPrompt += `сделай ${prompt}`;
            }
        }
