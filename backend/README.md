# Poligraf Backend (Go)

Backend сервер для Poligraf Mini App, написанный на Go.

## 🚀 Быстрый старт

### 1. Установка зависимостей

```bash
cd backend
go mod download
```

### 2. Настройка переменных окружения

Создайте файл `.env` (скопируйте из `.env.example`):

```bash
cp .env.example .env
```

Заполните API ключи:
```bash
V0_API_KEY=your_v0_api_key_here
```

### 3. Запуск сервера

```bash
go run main.go
```

Сервер запустится на `http://localhost:8080`

## 📁 Структура проекта

```
backend/
├── main.go              # Точка входа, настройка сервера
├── handlers/
│   └── generate.go      # Handler для /api/generate
├── go.mod               # Go модуль
├── .env.example         # Пример переменных окружения
└── README.md           # Документация
```

## 🔧 API Endpoints

### POST /api/generate

Генерирует код через v0.dev API (или другие провайдеры).

**Request:**
```json
{
  "userPrompt": "Создай визитку...",
  "image": "data:image/jpeg;base64,..." (опционально),
  "provider": "v0" | "lovable"
}
```

**Response:**
```json
{
  "result": "export default function...",
  "code": "export default function...",
  "provider": "v0.dev"
}
```

## 🛠️ Разработка

### Локальный запуск

```bash
# Запуск с hot reload (требует air)
air

# Или обычный запуск
go run main.go
```

### Установка air для hot reload

```bash
go install github.com/cosmtrek/air@latest
```

Создайте `.air.toml` в корне backend (опционально).

## 📦 Деплой

### Docker

```bash
# Сборка образа
docker build -t poligraf-backend .

# Запуск контейнера
docker run -p 8080:8080 --env-file .env poligraf-backend
```

### Railway / Render / Fly.io

Просто задеплойте папку `backend/` с указанием команды запуска:
```bash
go run main.go
```

Или используйте Dockerfile.

## 🔐 Переменные окружения

- `V0_API_KEY` - API ключ v0.dev (обязательно)
- `LOVABLE_API_KEY` - API ключ Lovable (опционально)
- `OPENAI_API_KEY` - API ключ OpenAI (опционально, для fallback)
- `PORT` - Порт сервера (по умолчанию 8080)

## 🧪 Тестирование

```bash
# Запуск тестов
go test ./...

# Запуск с покрытием
go test -cover ./...
```

## 📝 TODO

- [ ] Добавить поддержку Lovable API
- [ ] Добавить интеграцию с БД
- [ ] Добавить файловое хранилище
- [ ] Добавить логирование (structured logging)
- [ ] Добавить метрики
- [ ] Добавить rate limiting


